...
test('static URL stays same-origin so cross-origin isolation cannot block the frame', () => {
  // Under COEP a cross-origin nested document must send COEP or CORP of its own, which the
  // public bucket does not. Framing the artifact through the asset proxy keeps it same-origin.
  assert.equal(
    staticPreviewUrl({ sourceHash: 'sha256:abc', entry: 'nested/index.html' }),
    '/api/assets/previews/sha256%3Aabc/nested/index.html',
  );
});
