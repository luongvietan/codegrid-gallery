...
test('surfaces a pinned-down Next version as a notice for the whole run', async () => {
  const updates = [];

  await runRuntimePreview({
    zip: fakeZip(),
    signal: new AbortController().signal,
    prepare: () => ({ ...runtimeProject(), nextDowngradedTo: '15' }),
    boot: async () => { throw new Error('boot refused'); },
    onUpdate(snapshot) { updates.push(snapshot); },
  });

  assert.equal(updates[0].notice, null, 'nothing is known before the project is prepared');
  assert.ok(updates.slice(1).every((snapshot) => snapshot.notice?.includes('Next.js 15')));
