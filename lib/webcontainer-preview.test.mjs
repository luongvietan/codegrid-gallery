...
test('boot timeout releases retry while a late container is torn down without stale effects', async () => {
  const abort = new AbortController();
  const boot = deferred();
  const updates = [];
  const bootOrder = [];
  ...
  const retryAbort = new AbortController();
  let retryReady = false;
  let retryTeardownCount = 0;
  const retryRun = runRuntimePreview({
    zip: fakeZip(),
    signal: retryAbort.signal,
    prepare: runtimeProject,
    timeouts: FAST_TIMEOUTS,
    boot: async () => {
      bootOrder.push('retry');
      return {
        async mount() {},
        on(event, listener) {
          if (event === 'server-ready') queueMicrotask(() => listener(3000, 'https://retry.local'));
  ...
  const retrySettledBeforeGuard = await settleBeforeGuard(retryRun, retryAbort);
  ...
  assert.equal(retrySettledBeforeGuard, true);
  assert.deepEqual(bootOrder, ['first', 'retry']);
  assert.equal(retryReady, true);
  assert.equal(retryTeardownCount, 1);
  assert.equal(updates.at(-1).phase, 'failure');
...
test('cancels a stale boot without post-cancel updates while the replacement starts immediately', async () => {
  ...
  assert.deepEqual(lifecycle, [
    'boot:first', 'boot:second', 'teardown:first', 'teardown:second',
  ]);
});
