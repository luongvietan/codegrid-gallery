...
test('times out boot and tears down a container that resolves after the deadline', async () => {
  const abort = new AbortController();
  const boot = deferred();
  const updates = [];
  let teardownCount = 0;
  ...
  boot.resolve({
    async mount() {},
    on() { return () => {}; },
    async spawn() { return processResult(); },
    teardown() { teardownCount += 1; },
  });
...
test('cancels a stale boot without post-cancel updates and serializes the next boot behind teardown', async () => {
  ...
  function container(name, readyUrl) {
    liveContainers += 1;
    maxLiveContainers = Math.max(maxLiveContainers, liveContainers);
    return {
      async mount() {},
      on(event, listener) {
  ...
  assert.equal(maxLiveContainers, 1);
});
