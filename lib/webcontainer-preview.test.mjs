...
test('reports an unsupported template verbatim without booting or offering a retry', async () => {
  const updates = [];
  let boots = 0;
  ...
  await runRuntimePreview({
    ...
    prepare: () => { throw new UnsupportedRuntimeError(RUNTIME_UNSUPPORTED_NEXT_ERROR); },
    boot: async () => { boots += 1; throw new Error('should never boot'); },
    onUpdate(snapshot) { updates.push(snapshot); },
  });

  assert.equal(boots, 0);
  assert.equal(updates.at(-1).phase, 'failure');
  assert.equal(updates.at(-1).message, RUNTIME_UNSUPPORTED_NEXT_ERROR);
  assert.equal(updates.at(-1).recovery, null);
  assert.equal(runtimeRecoveryPolicy(updates.at(-1).recovery), null);
});
