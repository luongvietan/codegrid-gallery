...
const {
  ...
  NEXT_DOWNGRADE_RANGE,
} = await import('./webcontainer-runtime.ts');
...
function mountedManifest(prepared, root = 'package.json') {
  const segments = root.split('/');
  let node = prepared.files;
  for (const segment of segments.slice(0, -1)) node = node[segment].directory;
  return JSON.parse(new TextDecoder().decode(node[segments.at(-1)].file.contents));
}

test('pins an unrunnable Next major down to the newest one WebContainer can render', () => {
  const prepared = prepareRuntimeProject(zip({
    'package.json': JSON.stringify({
      dependencies: { next: '16.2.3', react: '19.2.4' },
      devDependencies: { 'eslint-config-next': '16.2.3' },
      scripts: { dev: 'next dev' },
    }),
  }));

  assert.equal(prepared.nextDowngradedTo, NEXT_DOWNGRADE_RANGE);
  const manifest = mountedManifest(prepared);
  assert.equal(manifest.dependencies.next, NEXT_DOWNGRADE_RANGE);
  assert.equal(manifest.dependencies.react, '19.2.4');
  assert.equal(manifest.devDependencies['eslint-config-next'], undefined);
});

test('downgrades only Next ranges guaranteed to install major 16 or newer', () => {
  for (const nextVersion of ['^16.2.3', '~16.2.3', '>=16', '17.0.0']) {
    const prepared = prepareRuntimeProject(zip({
      'package.json': JSON.stringify({
    ...
    assert.equal(prepared.nextDowngradedTo, NEXT_DOWNGRADE_RANGE, nextVersion);
  }
});

test('strips bundler flags the downgraded Next would reject', () => {
  const prepared = prepareRuntimeProject(zip({
    'package.json': JSON.stringify({
      dependencies: { next: '16.0.1' },
      scripts: { dev: 'next dev --webpack --turbopack', build: 'next build --turbopack' },
    }),
  }));
  ...
  assert.deepEqual(prepared.devCommand, ['npm', 'run', 'dev', '--', '--hostname', '0.0.0.0']);
});

test('drops a lockfile that no longer matches the downgraded manifest', () => {
  const prepared = prepareRuntimeProject(zip({
    'package.json': JSON.stringify({
  ...
  assert.deepEqual(
    prepared.installCommand,
    ['npm', 'install', '--ignore-scripts', '--no-audit', '--no-fund'],
  );
  assert.equal(prepared.files['package-lock.json'], undefined);
});

test('downgrades a Next 16 template that only exposes a start script', () => {
  const prepared = prepareRuntimeProject(zip({
    'package.json': JSON.stringify({
      dependencies: { next: '16.2.3' },
...
test('leaves a supported Next template untouched', () => {
  const prepared = prepareRuntimeProject(zip({
    'package.json': JSON.stringify({
      dependencies: { next: '15.1.0' },
      devDependencies: { 'eslint-config-next': '15.1.0' },
      scripts: { dev: 'next dev' },
    }),
    'package-lock.json': JSON.stringify({ lockfileVersion: 3 }),
  }));

  assert.equal(prepared.nextDowngradedTo, null);
  const manifest = mountedManifest(prepared);
  assert.equal(manifest.dependencies.next, '15.1.0');
  assert.equal(manifest.devDependencies['eslint-config-next'], '15.1.0');
  assert.deepEqual(
    prepared.installCommand,
...
test('still prepares Next ranges that may include older majors or are ambiguous', () => {
  for (const nextVersion of ['<16', '<=16', '>=15', '^16 || ^15', 'latest']) {
    const prepared = prepareRuntimeProject(zip({
      'package.json': JSON.stringify({
        dependencies: { next: nextVersion },
        scripts: { dev: 'next dev' },
...
test('keeps a Next 13 dev command compatible by omitting the webpack flag', () => {
  const prepared = prepareRuntimeProject(zip({
    'package.json': JSON.stringify({
      dependencies: { next: '13.5.6' },
      scripts: { dev: 'next dev' },
    }),
  }));
...
test('keeps supported Next start and non-Next dev commands unchanged', () => {
