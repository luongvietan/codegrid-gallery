...
const {
  ...
  RUNTIME_UNSUPPORTED_NEXT_ERROR,
...
test('refuses a Next 16 template instead of booting a runtime that cannot render it', () => {
  assert.throws(
    () => prepareRuntimeProject(zip({
      'package.json': JSON.stringify({
    ...
    (error) => (
      ...
      && error.message === RUNTIME_UNSUPPORTED_NEXT_ERROR
    ),
  );
});

test('refuses only Next ranges guaranteed to install major 16 or newer', () => {
  for (const nextVersion of ['^16.2.3', '~16.2.3', '>=16', '17.0.0']) {
    assert.throws(
      ...
      (error) => error instanceof UnsupportedRuntimeError,
      nextVersion,
    );
  }
});

test('refuses a Next 16 template that only exposes a start script', () => {
  assert.throws(
    () => prepareRuntimeProject(zip({
      'package.json': JSON.stringify({
        dependencies: { next: '16.2.3' },
